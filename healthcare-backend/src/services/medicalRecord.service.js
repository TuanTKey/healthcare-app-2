...
class MedicalRecordService {
  ...
  async getMedicalRecord(recordId) {
    try {
      ...
      // 🎯 NẾU KHÔNG TÌM THẤY, TRY TÌM THEO _id
      if (!medicalRecord) {
  ...
  async getAllMedicalRecords(filters = {}) {
    try {
      console.log('📋 [MEDICAL] Getting all medical records');
      ...
      console.error('❌ [MEDICAL] Get all medical records failed:', error.message);
      throw error;
    }
  }
  ...
  async updateMedicalRecord(recordId, updateData, updatedBy) {
    try {
      ...
      // 🎯 NORMALIZE DATA - Convert simple formats to complex formats
      const normalizeData = (updateData) => {
        const normalized = { ...updateData };

        // Handle symptoms: convert string array to object array if needed
        if (normalized.symptoms && Array.isArray(normalized.symptoms)) {
          normalized.symptoms = normalized.symptoms.map(symptom => {
            if (typeof symptom === 'string') {
              return { symptom, severity: 'MODERATE' };
            }
            return symptom;
          });
        }

        // Handle diagnoses: convert string array to object array if needed
        if (normalized.diagnoses && Array.isArray(normalized.diagnoses)) {
          normalized.diagnoses = normalized.diagnoses.map(diagnosis => {
            if (typeof diagnosis === 'string') {
              return { diagnosis, type: 'PRIMARY', certainty: 'PROBABLE' };
            }
            // Handle objects with 'description' field (from mobile app)
            if (diagnosis.description && !diagnosis.diagnosis) {
              return { ...diagnosis, diagnosis: diagnosis.description };
            }
            return diagnosis;
          });
        }

        // Handle vitalSigns: normalize to numeric values
        if (normalized.vitalSigns) {
          const vitals = normalized.vitalSigns;
          ...
          Object.keys(normalized.vitalSigns).forEach(key => {
            if (normalized.vitalSigns[key] === undefined) {
              delete normalized.vitalSigns[key];
            }
          });
        }

        // Handle treatmentPlan: if string, keep as is; if object, use recommendations
        if (normalized.treatmentPlan && typeof normalized.treatmentPlan === 'object' && !normalized.treatmentPlan.recommendations) {
          normalized.treatmentPlan = { recommendations: '' };
        }

        return normalized;
      };

      const cleanData = normalizeData(updateData);

      // 🎯 CẬP NHẬT THÔNG TIN
      const allowedFields = [
        'chiefComplaint', 'historyOfPresentIllness', 'symptoms', 'vitalSigns',
        'physicalExamination', 'diagnoses', 'treatmentPlan', 'notes', 'privacyLevel'
      ];
      
      allowedFields.forEach(field => {
        if (cleanData[field] !== undefined) {
          medicalRecord[field] = cleanData[field];
        }
      });
  ...
  async deleteMedicalRecord(recordId) {
    try {
      console.log('🗑️ [MEDICAL] Deleting medical record:', recordId);
      ...
      console.log('✅ [MEDICAL] Medical record deleted:', recordId);
      return { message: 'Hồ sơ bệnh án đã được xóa' };

    } catch (error) {
      console.error('❌ [MEDICAL] Delete medical record failed:', error.message);
      throw error;
    }
  }
