...
const medicalRecordValidation = {
  ...
  updateMedicalRecord: Joi.object({
    ...
    vitalSigns: Joi.alternatives().try(
      Joi.object({
        bloodPressure: Joi.string().optional(),
        heartRate: Joi.alternatives().try(Joi.string(), Joi.number()).optional(),
        respiratoryRate: Joi.alternatives().try(Joi.string(), Joi.number()).optional(),
        temperature: Joi.alternatives().try(Joi.string(), Joi.number()).optional(),
        oxygenSaturation: Joi.alternatives().try(Joi.string(), Joi.number()).optional(),
        height: Joi.alternatives().try(Joi.string(), Joi.number()).optional(),
    ...
    physicalExamination: Joi.string().max(2000).optional(),
    diagnoses: Joi.alternatives().try(
      Joi.array().items(Joi.string()),
      Joi.array().items(
        Joi.object({
          ...
          certainty: Joi.string().valid('CONFIRMED', 'PROBABLE', 'POSSIBLE').optional(),
          notes: Joi.string().optional()
        }).unknown(true) // Allow any additional fields
      )
    ).optional(),
    treatmentPlan: Joi.alternatives().try(
      Joi.string(),
      Joi.object({
